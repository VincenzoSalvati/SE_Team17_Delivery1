...
public class AssignHoursBrowseService {
    String name_maint = "";
    int skills = 0;
    int skillTot = 0;
    int h8to9 = 0;
    ...
    public String getAssignHoursBrowseToJSON(MySqlDbConnection db) {
        ...
        String assignMaintainerBrowseJSONFormat = "{\"maint\":\"{MAINT}\",\"skills\":\"{SKILLS}\",\"h8to9\":\"{h8to9}\",\"h9to10\":\"{h9to10}\",\"h10to11\":\"{h10to11}\",\"h11to12\":\"{h11to12}\",\"h14to15\":\"{h14to15}\",\"h15to16\":\"{h15to16}\",\"h16to17\":\"{h16to17}\"}";
        ...
        try {
            ...
            JSONRow = assignMaintainerBrowseJSONFormat.replace("{MAINT}", name_maint);
            JSONRow = JSONRow.replace("{SKILLS}", skills + "/" + skillTot);
    ...
    private void getAvailabilityHours(int maintainer, String day, int week) {
        try {
            ...
            ResultSet rs;
            rs = stmt.executeQuery("SELECT a.avail_8to9, a.avail_9to10, a.avail_10to11, a.avail_11to12, a.avail_14to15, a.avail_15to16, a.avail_16to17, m.name FROM maintainer AS m, availability AS a WHERE a.id_maint=m.id AND a.id_maint=" + maintainer + " AND a.day=\"" + day + "\"" + " AND a.week=\"" + week + "\"");
            while (rs.next()) {
                h8to9 = rs.getInt(1);
                h9to10 = rs.getInt(2);
                h10to11 = rs.getInt(3);
